...
from synchronization import synchronize_2d_tm, synchronize_1d_tm, synchronize_1d, synchronize_and_match_2d, synchronize_2d, synchronize_and_match_1d
...
def ppm_experiment(seed, image_idx, SNR, Ncopy, Ndir, use_sPCA):
    Coeff, Freqs, rad_freqs, Mean, Phi_ns, sigma, Coeff_raw, rotations = make_data(Ncopy=Ncopy,SNR=SNR,seed=seed,image_idx=image_idx, sPCA=use_sPCA)
    image0 = recover_image(Coeff_raw, Phi_ns, Freqs, r_max, Mean)
...
def ppm_em_experiment(seed, image_idx, SNR, Ncopy, Ndir, use_sPCA, use_signal_prior, gamma, BW, path_learning, P):
    Coeff, Freqs, rad_freqs, Mean, Phi_ns, sigma, Coeff_raw, rotations = make_data(Ncopy=Ncopy,SNR=SNR,seed=seed,image_idx=image_idx, sPCA=use_sPCA)
    image0 = recover_image(Coeff_raw, Phi_ns, Freqs, r_max, Mean)
    ...
    x, num_iter_synch_em, t_synch_em = EM_General_Prior(Coeff, Freqs, sigma, Ndir, 1/Ndir * np.ones((Ndir,1)), Ndir, 0, Coeff_raw, use_signal_prior, uniform=False)
    x = align_image(x, Coeff_raw, Freqs)
    imager = recover_image(x, Phi_ns, Freqs, r_max, Mean)
    err_synch_em = np.sqrt(np.sum(np.sum((image0 - imager) ** 2,axis=-1),axis=0)/np.sum(np.sum(image0 ** 2, axis=-1),axis=0))
...
def dist_est_em_experiment_1d(seed, sigma, L, b, Ncopy, use_signal_prior):
    np.random.seed(seed)
    ...
    x_init = np.random.randn(L, 1)
    print('start dist est em processing')
    t = time()
    x_est_em, rho_est_em, num_iter_em_uniform = em_1d(y, sigma, n_iter, tol, x_init, b=b_prior,
...
def synchronize_and_match_em_experiment(seed, sigma, L, b, Ncopy, use_signal_prior, P):
    np.random.seed(seed)
...
def synch_em_1d_experiment(seed, sigma, L, b, Ncopy, use_signal_prior, P, gamma):
    np.random.seed(seed)
    ...
    t = time()
    x_est_em, rho_est_em, num_iter_em_uniform = em_1d(y_s, sigma, n_iter, tol, x_init, b=b_prior,
                                                       rho_prior=(measured_rho,gamma), uniform=False)
    ...
    plt.savefig('est_dist/rho_est_L_%d_sigma_%.2f_N_%d_b_%d_seed_%d_SNR_%.2f.png'%(L,sigma,Ncopy,b,seed,1/sigma**2))
    plt.clf()
